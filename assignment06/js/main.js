...
colorBtnContainer.addEventListener("click", (event) => {
  changeColor(event);
  changeImageColor();
});
sizeBtnContainer.addEventListener("click", (event) => changeSize(event));
technologyBtnContainer.addEventListener("click", (event) =>
...
const changeImageColor = () => {
  ...
  images.forEach((shoe) => {
    const colorMatch =
      shoe.getAttribute("color") === activeColor.getAttribute("color");
